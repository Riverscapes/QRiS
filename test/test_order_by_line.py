...
class TestOrderByLineTask(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.layer = QgsVectorLayer("LineString?crs=EPSG:4326&field=name:string&field=sequence:integer", "cross_section_features", "memory")
        ...
        self.centerline_forward = QgsGeometry.fromWkt("LINESTRING(0 0, 1 1, 0 2, 1 3, 0 4)")
        self.centerline_reverse = QgsGeometry.fromWkt("LINESTRING(0 4, 1 3, 0 2, 1 1, 0 0)")

    def _sequence_by_name(self):
        return {f["name"]: f["sequence"] for f in self.layer.getFeatures()}

    def _build_sample_frame_layer(self):
        layer = QgsVectorLayer("Polygon?crs=EPSG:4326&field=fid:integer&field=name:string&field=display_label:string&field=flows_into:integer&field=flow_path:string", "sample_frame_features", "memory")
    ...
    def test_cross_sections_ordered_by_centerline_not_digitized_order(self):
        task = self.OrderByLineTask(layer_path="mock://cross_section_features", centerline=self.centerline_forward, label_field="sequence", chain_field=None)

        with patch("qris_dev.src.gp.order_by_line_task.QgsVectorLayer", return_value=self.layer):
    ...
    def test_reversing_centerline_reverses_sequence(self):
        task = self.OrderByLineTask(layer_path="mock://cross_section_features", centerline=self.centerline_reverse, label_field="sequence", chain_field=None)
    ...
    def test_epsg4326_meander_realistic_cross_section_ordering(self):
        ...
        layer = QgsVectorLayer("LineString?crs=EPSG:4326&field=name:string&field=sequence:integer", "cross_section_features_realistic", "memory")
        self.assertTrue(layer.isValid())
        ...
        centerline_forward = QgsGeometry.fromWkt("LINESTRING(-111.34395 42.94010, -111.34360 42.94065, -111.34315 42.94120, -111.34370 42.94175, -111.34305 42.94235, -111.34355 42.94295, -111.34295 42.94355)")
        centerline_reverse = QgsGeometry.fromWkt("LINESTRING(-111.34295 42.94355, -111.34355 42.94295, -111.34305 42.94235, -111.34370 42.94175, -111.34315 42.94120, -111.34360 42.94065, -111.34395 42.94010)")

        task = self.OrderByLineTask(layer_path="mock://cross_section_features_realistic", centerline=centerline_forward, label_field="sequence", chain_field=None)

        with patch("qris_dev.src.gp.order_by_line_task.QgsVectorLayer", return_value=layer):
            self.assertTrue(task.run(), msg=str(task.exception))
        ...
        task = self.OrderByLineTask(layer_path="mock://cross_section_features_realistic", centerline=centerline_reverse, label_field="sequence", chain_field=None)
    ...
    def test_sample_frames_order_and_flows_into_chain(self):
        ...
        task = self.OrderByLineTask(
            layer_path="mock://sample_frame_features", centerline=self.centerline_forward, label_field="display_label", chain_field="flows_into", flow_path_field="flow_path", flow_path_value="Centerline A", intersecting_only=True
        )
    ...
    def test_sample_frames_reverse_centerline_reverses_order_and_chain(self):
        ...
        task = self.OrderByLineTask(
            layer_path="mock://sample_frame_features", centerline=self.centerline_reverse, label_field="display_label", chain_field="flows_into", flow_path_field="flow_path", flow_path_value="Centerline B", intersecting_only=True
        )

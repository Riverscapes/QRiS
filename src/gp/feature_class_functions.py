...
def import_existing(source_path: str, dest_path: str, dest_layer_name: str, output_id: int, output_id_field: str, attributes: Optional[dict] = None, clip_mask: Optional[tuple] = None) -> None:
    """
...
def browse_raster(parent, description: str) -> str:
    ...
    if uri is not None and uri.isValid():
        ...
        if uri.providerKey != "gdal":
            QMessageBox.warning(parent, "Invalid Raster", "This raster format is not currently supported by QRiS. Please select a different raster.")
            return None
        # if uri extension is .vrt then it is a virtual raster and cannot be used
        vrt_ext = os.path.splitext(uri.uri)[1].lower()
        if vrt_ext == ".vrt":
            QMessageBox.warning(parent, "Invalid Raster", "The raster is a virtual raster and cannot be used. Please select a different raster.")
            return None
        return uri.uri
...
def browse_vector(parent, description: str, geometry_type: QgsWkbTypes.GeometryType) -> str:
    ...
    if uri is not None and uri.isValid():
        ...
        if geometry_type is not None and geometry_type != layer.geometryType():
            QMessageBox.warning(parent, "Invalid Geometry Type", f"The layer is of geometry type {QgsWkbTypes.geometryDisplayString(layer.geometryType())} but must be of type {QgsWkbTypes.geometryDisplayString(geometry_type)}.")
            return None

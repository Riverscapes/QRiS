...
class VicinityMapExportTask(QgsTask):
    ...
    def _resolve_centroid_point_wgs84(self) -> QgsPointXY:
        ...
        for label, source_ids in centroid_sources:
            ...
            layer = build_aoi_centroids_layer(self.qris_project.project_file, source_ids, layer_name=f"{label} Centroids")
        ...
        if centroid_layer is None:
            ...
            raise Exception(f"Cannot generate vicinity map centroid: available geometry sources did not contain valid polygon features ({', '.join(attempted_sources)}).")
        ...
        transform = QgsCoordinateTransform(centroid_crs, QgsCoordinateReferenceSystem("EPSG:4326"), QgsProject.instance().transformContext())
    ...
    def _resolve_intersecting_state(self, centroid_point_wgs84: QgsPointXY) -> tuple[str, QgsGeometry]:
        ...
        if states_crs.isValid() and states_crs != wgs84:
            to_states = QgsCoordinateTransform(wgs84, states_crs, QgsProject.instance().transformContext())
        ...
        for state_feature in states_layer.selectedFeatures():
            ...
            if states_crs.isValid() and states_crs != wgs84:
                to_wgs84 = QgsCoordinateTransform(states_crs, wgs84, QgsProject.instance().transformContext())
                result_geom.transform(to_wgs84)

            return state_name, result_geom

        self.exception = Exception("Project location is outside supported regions. Cannot generate vicinity map. (No containing state found for centroid.)")
        return None, None
    ...
    def _build_region_render_layer(self, region_geometry: QgsGeometry, region_label: str) -> QgsVectorLayer:
        ...
        symbol = QgsFillSymbol.createSimple({"color": "240,240,240,185", "outline_color": "0,0,0,255", "outline_width": "1.4"})
    ...
    def _build_centroid_render_layer(self, centroid_point: QgsPointXY) -> QgsVectorLayer:
        ...
        symbol = QgsMarkerSymbol.createSimple({"name": "star", "color": "0,0,0", "outline_color": "0,0,0", "outline_width": "0.4", "size": "5"})

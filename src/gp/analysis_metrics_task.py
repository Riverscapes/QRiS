...
class AnalysisMetricsTask(QgsTask):
    ...
    def _queue_metric_value_row(self, pending_rows: list, event_id: int, sample_frame_id: int, metric_value: MetricValue, unit_id: Optional[int] = None):
        pending_rows.append(
            (
                self.analysis.id,
                event_id,
    ...
    def _resolve_runtime_dependency_values(metric, metric_values: dict) -> dict:
        ...
        values_by_exact = {}
        values_by_loose = {}
        for _metric_id, metric_value in metric_values.items():
        ...
        for dep in dependencies:
            ...
            if not dep_machine:
                raise MetricInputMissingError(f"Metric '{metric.machine_name}' has dependency missing metric_id_ref")
            ...
            if dep_version is not None:
                ...
                if key is None:
                    raise MetricInputMissingError(f"Missing dependency value for '{dep_protocol}.{dep_machine}' version '{dep_version}'")
                ...
                if len(candidates) < 1:
                    raise MetricInputMissingError(f"Missing dependency value for '{dep_protocol}.{dep_machine}'")
                if len(candidates) > 1:
                    raise MetricInputMissingError(f"Ambiguous dependency value for '{dep_protocol}.{dep_machine}', specify version")
    ...
    def _resolve_dependency(dep: dict, metric, exact_index: dict, loose_index: dict):
        ...
        if not ref_machine:
            raise ValueError(f"Invalid dependency definition on metric '{metric.machine_name}': missing metric_id_ref")
        ...
        if ref_version is not None:
            ...
            if resolved is None:
                raise ValueError(f"Metric '{metric.machine_name}' depends on '{ref_protocol}.{ref_machine}' version '{ref_version}' which is not available in this analysis")
            return resolved

        key = AnalysisMetricsTask._dependency_key(ref_machine, ref_protocol, None)
        candidates = loose_index.get(key, [])
        if len(candidates) < 1:
            raise ValueError(f"Metric '{metric.machine_name}' depends on '{ref_protocol}.{ref_machine}' which is not available in this analysis")
        if len(candidates) > 1:
            raise ValueError(f"Metric '{metric.machine_name}' dependency '{ref_protocol}.{ref_machine}' is ambiguous; specify dependency version")
        return candidates[0]
    ...
    def plan_metric_execution(analysis_metrics: dict, requested_metric_ids: list) -> list:
        ...
        order_index = {metric_id: idx for idx, metric_id in enumerate([am.metric.id for am in available])}

...
class ZoiTypeDlg(QDialog, DIALOG_CLASS):
    ...
    def __init__(self, qris_project):
        ...
        curs = conn.cursor()
        curs.execute('SELECT * FROM lkp_zoi_influence')
        influences = curs.fetchall()
        conn.close()
        for influence in influences:
            self.comboBox_zoi_influence.addItem(influence[1], influence[0])
    ...
    def save_zoi_type(self):
        ...
        new_zoi_influence = self.comboBox_zoi_influence.currentData()
        ...
        new_zoi_type_feature.setAttribute("influence_id", new_zoi_influence)

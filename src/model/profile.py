...
class Profile(DBItem):
    ...
    def __init__(self, id: int, name: str, profile_type_id: int, description: str):
        ...
        self.id_column_name = 'profile_id'
        self.icon = 'gis'
...
def insert_profile(db_path: str, name: str, profile_type_id: int, description: str) -> Profile:
    ...
    with sqlite3.connect(db_path) as conn:
        try:
            curs = conn.cursor()
            curs.execute('INSERT INTO profiles (name, profile_type_id, description) VALUES (?, ?, ?)', [name, profile_type_id, description])
            id = curs.lastrowid

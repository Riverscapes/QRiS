...
class FrmSampleFrame(QDialog):
    ...
    def __init__(self, parent, qris_project: Project, import_source_path: Optional[str] = None, sample_frame: SampleFrame = None, create_sample_frame: bool = False):
        ...
        self.setWindowTitle("Create New (Empty) Sample Frame")
        ...
        if self.import_source_path is not None:
            if isinstance(self.import_source_path, QgsVectorLayer):
                ...
                self.setWindowTitle("Import New Sample Frame From Temporary Layer")
            else:
                self.tab_inputs = SampleFrameInputs(self, self.qris_project, self.import_source_path)
                self.tab_attributes = SampleFrameAttributesAddFields(self, self.import_source_path)
                self.setWindowTitle("Import New Sample Frame From Feature Class")
        else:
        ...
        if create_sample_frame is True:
            ...
            self.setWindowTitle("Create New Sample Frame from Existing Layers")
    ...
    def on_split_complete(self, result: bool):
        ...
        if self.centerline_geom is not None:
            order_task = OrderByLineTask(
                f"{self.qris_project.project_file}|layername=sample_frame_features", self.centerline_geom, filter_expression=f"sample_frame_id = {self.sample_frame.id}", label_field="display_label", chain_field="flows_into"
            )
        ...
        try:
            with sqlite3.connect(self.qris_project.project_file) as conn:
                ...
                if is_sequence:
                    ...
                    ordered = sorted(((fid, int(meta["source_topology"])) for fid, meta in feature_meta.items() if meta.get("source_topology") is not None), key=lambda x: x[1])
                else:
                    # Derive order by traversing the source topology chain
                    # Build source_fid → dest_fid lookup
                    source_fid_to_dest = {str(meta["source_fid"]): fid for fid, meta in feature_meta.items() if meta.get("source_fid") is not None}
                    # Build source_fid → downstream source_fid map
                    src_to_next = {str(meta["source_fid"]): str(meta["source_topology"]) for meta in feature_meta.values() if meta.get("source_fid") is not None and meta.get("source_topology") is not None}
...
class SampleFrameInputs(QWidget):
    ...
    def setupUi(self):
        ...
        self.cboTopologyField.currentIndexChanged.connect(lambda idx: topology_type_widget.setEnabled(idx > 0))

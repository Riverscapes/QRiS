...
class FrmMetricAvailabilityMatrix(QtWidgets.QDialog):
    def __init__(
        ...
        highlight_dce_id: Optional[int] = None,
        limit_dces: Optional[list] = None,
        analysis=None,
        selected_analysis_metrics: Optional[dict] = None,
    ):
    ...
    def load_data(self):
        ...
        for layer_def in dce_layers:
            ...
            if metric_protocol and metric_protocol.protocol_layers:
                ...
                for layer in metric_protocol.protocol_layers.values():
                    if layer.layer_id == ref:
        ...
        for row_idx, event in enumerate(events):
            ...
            # 2. Check each column
            for col_idx, (c_type, ref, _display_header, _) in enumerate(columns_info):
                ...
                if c_type == "input":
                    ...
                    if found_id is not None:
                        ...
                        if found_obj and hasattr(found_obj, "feature_count"):
                            try:
                                ...
                                status_item.setToolTip(f"Error checking features: {e!s}")
                    else:
                    ...
                    selected_by_exact = {f"{m.protocol_machine_code}::{m.machine_name}::{m.version}": m for m in selected_metric_objs}
                    selected_by_loose = {}

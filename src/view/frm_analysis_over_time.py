...
class AnalysisOverTimeChart(QtWidgets.QWidget):
    ...
    def setup_ui(self):
        ...
        self.cbo_value_type.addItems(["Absolute", "Relative to Minimum", "Relative to Maximum", "Relative to Mean"])
    ...
    def get_export_data(self):
        ...
        for i, label in enumerate(x_labels):
            ...
            export_list.append({"Event": label, "Value": float(val), "Unit": ylabel, "Uncertainty": float(err)})
        return export_list
        ...
        if metric_values and len(metric_values) == len(y_plot):
            for i, y in enumerate(y_plot):
                if np.isfinite(y) and metric_values[i]:
                    self.metric_points.append({"x": i, "y": y, "data": metric_values[i]})

        mask = np.isfinite(y_plot)
        if np.any(mask):
            self.ax.errorbar(np.array(x_indices)[mask], np.array(y_plot)[mask], yerr=np.array(y_err_plot)[mask] if y_err else None, fmt="o", capsize=5, ecolor="red")

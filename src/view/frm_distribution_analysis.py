...
class FrmDistributionAnalysis(QtWidgets.QDialog):
    ...
    def get_layer_colors(self, event_layer, attribute_name):
        colors = {}
        if not self.map_manager:
            return colors
            
        try:
            qml_path = self.map_manager.get_symbology_qml(event_layer.layer.qml)
            if qml_path and QtCore.QFile.exists(qml_path):
                ...
                if renderer is not None:
                     ...
                     if categories is not None and symbols is not None:
                         # Map symbol name to color
                         symbol_colors = {}
                         for sym in symbols.findall("symbol"):
                             layer = sym.find("layer")
                             if layer:
                                 for prop in layer.findall("prop"):
                                     if prop.get("k") in ["color", "line_color", "fill_color"]:
                                         symbol_colors[sym.get("name")] = prop.get("v")
                                         break
                         
                         for cat in categories.findall("category"):
                             val = cat.get("value")
                             sym_ref = cat.get("symbol")
                             if sym_ref in symbol_colors:
                                 rgba = symbol_colors[sym_ref].split(',')
                                 if len(rgba) >= 3:
                                     colors[str(val)] = QtGui.QColor(int(rgba[0]), int(rgba[1]), int(rgba[2]))
        except Exception as e:
            print(f"Error parsing QML: {e}")

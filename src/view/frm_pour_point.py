...
from .utilities import add_standard_form_buttons, format_superscript, validate_name
...
class FrmPourPoint(QtWidgets.QDialog):
    ...
    def load_basin_characteristics(self):
        if self.pour_point.basin_chars is not None and "parameters" in self.pour_point.basin_chars:
            ...
            for param in self.pour_point.basin_chars["parameters"]:
                ...
                basin_data.append([param.get("name", ""), code, format_superscript(param.get("unit", "")), val, param.get("override_value", "")])
    ...
    def show_flow_context_menu(self, position):
        ...
        if val_item:
            ...
            if raw_val:
                ...
                menu.addAction(QtGui.QIcon(":/plugins/qris_toolbar/copy_content"), "Copy Value", lambda: QtWidgets.QApplication.clipboard().setText(str(raw_val)))
    ...
    def show_basin_context_menu(self, position):
        ...
        if val_str and val_str != "N/A":
            menu.addSeparator()
            menu.addAction(QtGui.QIcon(":/plugins/qris_toolbar/copy_content"), "Copy Value", lambda: QtWidgets.QApplication.clipboard().setText(val_str))
                ...
                for region in estimate.get("regressionRegions", []):
                    ...
                    if has_param:
                        ...
                        if stat_names:
                            ...
                            usage.append("Stats:    (All statistics in this region)")
                        usage.append("-" * 30)
    ...
    def show_flow_stat_details(self, item):
        ...
        if scenario_data and "parameters" in scenario_data:
            ...
            for p in scenario_data["parameters"]:
                ...
                details += f"{p.get('name', '')[:33]:<35} | {p.get('code', ''):<10} | {p.get('value', 'N/A')!s:<10} | {lim_str}\n"
        else:
    ...
    def download_basin_characteristics(self):
        ...
        try:
            ...
            if not state_code:
                ...
                state_code, _status = get_state_from_coordinates(self.pour_point.latitude, self.pour_point.longitude)
    ...
    def recalculate_flow_stats(self):
        ...
        if getattr(self, "basin_model", None) and self.pour_point.basin_chars and "parameters" in self.pour_point.basin_chars:
            for _i, row in enumerate(self.basin_model._data):  # row is [name, code, units, value, override_value]
                code = row[1]
                override_val = row[4]
                for param in self.pour_point.basin_chars["parameters"]:
        ...
        if self.pour_point is not None:
            try:
                ...
                if "unique" in str(ex).lower():
                    QtWidgets.QMessageBox.warning(self, "Duplicate Name", f"A pour point with the name '{self.txtName.text()}' already exists. Please choose a unique name.")

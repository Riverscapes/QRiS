...
class FrmProfile(QtWidgets.QDialog):
    def __init__(self, parent, qris_project: Project, import_source_path, profile: Profile = None, profile_type=None, fc_name: str = "profile_features", system_metadata: Optional[dict] = None, metrics: Optional[dict] = None):
        ...
        self.setWindowTitle("Create New Profile" if self.profile is None else "Edit Profile Properties")
        ...
        if self.profile is not None:
            try:
                ...
                if "unique" in str(ex).lower():
                    QtWidgets.QMessageBox.warning(self, "Duplicate Name", f"A profile with the name '{self.txtName.text()}' already exists. Please choose a unique name.")
                    self.txtName.setFocus()
                else:

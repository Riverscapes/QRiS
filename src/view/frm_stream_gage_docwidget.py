...
class FrmStreamGageDocWidget(QtWidgets.QDockWidget):
    ...
    def load_discharge_data(self):
        ...
        with sqlite3.connect(self.project.project_file) as conn:
            curs = conn.cursor()
            curs.execute(
                "SELECT measurement_date, discharge FROM stream_gage_discharges WHERE (stream_gage_id = ?) AND (measurement_date >= ?) AND (measurement_date < ?) ORDER BY measurement_date",
    ...
    def delete_gage(self):
        ...
        if self._static_ax is not None and self.static_canvas is not None:
            self._static_ax.cla()
            self.static_canvas.draw()
    ...
    def setupUi(self):
        ...
        self.export_widget = ChartExportWidget(
            ...
            get_figure_callback=(lambda: self.static_canvas.figure if self.static_canvas is not None else None),

        ...
        if event is not None:
            ...
            if "system" in self.metadata_widget.metadata:
                ...
                if "constructionDate" in self.metadata_widget.metadata["system"]:
                    ...
                    construction_date = DateSpec(construction_date_dict.get("year", None), construction_date_dict.get("month", None), construction_date_dict.get("day", None))
            ...
            for p, layer in self.layer_widget.available_layers:
                if "structure_points" in layer.id:
    ...
    def accept(self):
        ...
        for p, layer in self.layer_widget.available_layers:
            ...
            if self.layer_widget.current_layers_state.get(key, False):
                selected_layers.append(layer.id)

        if any("structure_points" in layer_id or "structure_lines" in layer_id for layer_id in selected_layers) is False:
        ...
        dict_construction_date = {"year": construction_date.year, "month": construction_date.month, "day": construction_date.day}

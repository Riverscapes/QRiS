...
class FrmClimateEngineExplorer(QtWidgets.QDockWidget):
    def __init__(self, parent: QtWidgets.QWidget, qris_project: Project, qris_map_manager: QRisMapManager):
        ...
        self.sample_frame_widget = SampleFrameWidget(
            self, self.qris_project, self.qris_map_manager, first_index_empty=True, sample_frame_types=[SampleFrame.AOI_SAMPLE_FRAME_TYPE, SampleFrame.VALLEY_BOTTOM_SAMPLE_FRAME_TYPE, SampleFrame.SAMPLE_FRAME_TYPE]
        )
    ...
    def create_plot(self):
        ...
        for event_date, event_name in event_dates:
            ...
            if self.chk_event_labels.isChecked():
                ...
                self._static_ax.annotate(event_name, xy=(date, y_pos), xytext=(0, 5), textcoords="offset points", ha="right", va="bottom", fontsize=9, rotation=90, color="black", fontweight="bold")
        ...
        sample_frame_feature_ids = self.sample_frame_widget.get_selected_sample_frame_feature_ids()
        _frame = self.sample_frame_widget.selected_sample_frame()

        with sqlite3.connect(self.qris_project.project_file) as conn:
            curs = conn.cursor()
            curs.execute("SELECT * FROM time_series WHERE time_series_id = ?", (time_series_id,))
            time_series = curs.fetchone()
            _time_series_name = time_series[1]
            ...
            for sample_frame_feature_id in sample_frame_feature_ids:
                curs.execute("SELECT time_value, value FROM sample_frame_time_series WHERE time_series_id = ? AND sample_frame_fid = ? AND time_value BETWEEN ? AND ?", (time_series_id, sample_frame_feature_id, start_date, end_date))
    ...
    def export_data(self):
        """Backward-compatible export entry point."""
        self.export_widget.export_values()
    ...
    def get_export_data(self):
        ...
        with sqlite3.connect(self.qris_project.project_file) as conn:
            ...
            _variable_id = metadata.get("variable", "unknown_variable")
            _dataset_name = self.datasets[dataset_id]["datasetName"] if dataset_id in self.datasets else dataset_id
            _y_label = metadata["units"] if "units" in metadata else "Value"
            for sample_frame_feature_id in sample_frame_feature_ids:
                curs.execute("SELECT time_value, value FROM sample_frame_time_series WHERE time_series_id = ? AND sample_frame_fid = ? AND time_value BETWEEN ? AND ?", (time_series_id, sample_frame_feature_id, start_date, end_date))
                data[sample_frame_feature_id] = [(datetime.strptime(row[0], "%Y-%m-%d"), row[1]) for row in curs.fetchall()]
                curs.execute("SELECT display_label FROM sample_frame_features WHERE fid = ?", (sample_frame_feature_id,))
                label_row = curs.fetchone()
                if label_row is not None and label_row[0] not in [None, ""]:
                    feature_labels[sample_frame_feature_id] = label_row[0]
                else:
                    feature_labels[sample_frame_feature_id] = str(sample_frame_feature_id)

        rows = []
        for sample_frame_feature_id in sample_frame_feature_ids:
            values = data.get(sample_frame_feature_id, [])
            feature_name = feature_labels.get(sample_frame_feature_id, str(sample_frame_feature_id))
            for date_value, metric_value in values:
                rows.append(
                    {
                        "date": date_value.strftime("%Y-%m-%d"),
                        "sample_frame_feature_id": sample_frame_feature_id,
                        "sample_frame_feature": feature_name,
                        "value": metric_value,
    ...
    def setupUi(self):
        ...
        self.export_widget = ChartExportWidget(
            ...
            export_type="climate_engine_timeseries",
        )
        self.horiz_chart_controls.addWidget(self.export_widget)

...
class DistributionAnalysisWidget(QtWidgets.QWidget):
    ...
    def on_layer_changed(self):
        ...
        for field in fields:
            ...
            is_categorical = "text" in f_type or "string" in f_type or f_type == "list" or "lookup" in field or "values" in field or field.get("widget") == "value_map"
    ...
    def update_units_menu(self):
        ...
        units = []
        if geom_type == "Linestring":
            units = [("Meters (m)", 1.0, "m"), ("Kilometers (km)", 0.001, "km"), ("Feet (ft)", 3.28084, "ft"), ("Miles (mi)", 0.000621371, "mi")]
        elif geom_type == "Polygon":
            units = [("Sq Meters (m²)", 1.0, "m²"), ("Sq Kilometers (km²)", 1e-6, "km²"), ("Hectares (ha)", 0.0001, "ha"), ("Sq Feet (ft²)", 10.7639, "ft²"), ("Acres (ac)", 0.000247105, "ac"), ("Sq Miles (mi²)", 3.861e-7, "mi²")]
        else:
            units = [("Count", 1.0, "")]

        for name, factor, label in units:
            action = menu.addAction(name)
            action.triggered.connect(lambda checked, f=factor, lab=label: self.set_unit(f, lab))
    ...
    def get_export_data(self):
        ...
        if len(data) >= 6:
            distribution, total_mapped, _event_layer, _metric_field, scope_measure, measure_type = data[:6]
        elif len(data) == 5:
            distribution, total_mapped, _event_layer, _metric_field, scope_measure = data[:5]
        ...
        for k in sorted_keys:
            ...
            export_list.append({"Category": k, "Value": float(val), "Unit": unit, "Percentage": float(pct)})
    ...
    def get_sorted_attribute_keys(self, keys):
        ...
        if self.attribute_sort_mode == "count_desc":
            return sorted(key_texts, key=lambda k: (-self.current_attribute_feature_counts.get(k, 0), k.lower()))
    ...
    def calculate_distribution(self):
        ...
        self.current_distribution_data = (distribution, total_amount, event_layer, metric_field, scope_measure, measure_type)
    ...
    def draw_chart(self):
        ...
        if chart_type == "stacked":
            ...
            legend = ax.legend(loc="upper center", bbox_to_anchor=(0.5, -0.25), ncol=2, frameon=False, prop=font_settings)
...
class ChartSettingsDialog(QtWidgets.QDialog):
    ...
    def get_settings(self):
        return {"font": self.font, "show_pct": self.chkShowPct.isChecked(), "pct_basis": self.cmbBasis.currentData()}

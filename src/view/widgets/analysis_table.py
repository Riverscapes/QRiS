...
class MetricStatusWidgetButtons(QtWidgets.QWidget):
...
class AnalysisTable(QtWidgets.QWidget):
    ...
    column: ClassVar[dict] = {"status": 0, "protocol": 1, "metric": 2, "value": 3, "units": 4, "uncertainty": 5}
    ...
    def create_row(self, row, analysis_metric: AnalysisMetric):
        ...
        # Status Widget
        status_widget = MetricStatusWidgetButtons()
    ...
    def load_values(self, event, mask_feature_id):
        ...
        can_load = mask_feature_id is not None and self.qris_project is not None and (event is not None or (self.analysis is not None and self.analysis.is_simple_intrinsic_mode()))

...
# from ..model.layout import delete_layout, get_layout_xml, get_project_layouts
...
class FrmMapTemplatePicker(QtWidgets.QDialog):
    ...
    def load_system_templates(self):
        ...
        self.protocols_dir = os.path.join(plugin_root, "resources", "map_templates")
        ...
        if os.path.exists(self.protocols_dir):
            ...
            for f in files:
                ...
                item.setData(USER_ROLE, full_path)
    ...
    #     confirm = QtWidgets.QMessageBox.question(self, "Confirm Delete", f"Are you sure you want to delete layout '{item.text()}'?", QtWidgets.QMessageBox.Yes | QtWidgets.QMessageBox.No)
    ...
    def accept(self):
        if self.tabs.currentIndex() == 0:
            if len(self.list_system.selectedItems()) > 0:
                self.selected_template = self.list_system.selectedItems()[0].data(USER_ROLE)
                self.selected_type = "file"
                super().accept()
        else:
            pass
            # if self.project_file and len(self.list_project.selectedItems()) > 0:
            #     layout_id = self.list_project.selectedItems()[0].data(USER_ROLE)

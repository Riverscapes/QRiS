...
A [GeoPackage](https://www.geopackage.org/) is an open standard for storing geospatial data published by the [Open Geospatial Consortium](https://www.ogc.org/) (OGC) with numerous benefits compared to the [ShapeFile](http://switchfromshapefile.org/) format. It consists of a single file that can store multiple vector feature classes and raster datasets. The file is actually a SQLite database with a specific schema. 

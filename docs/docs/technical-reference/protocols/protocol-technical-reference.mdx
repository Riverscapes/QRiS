...
## Protocol XML
Each Protocol is be defined by a separate XML file. Protocols that ship with QRiS reside in the [RiverscapesXML repository](https://github.com/Riverscapes/RiverscapesXML). An XSD “rule file” defines the schema for these XML files and a GitHub “action” used to ensure that only protocols that validate against this rule file are be permitted into the git repository. Protocols can also be develoed and deployed locally by placing the protocol XML in the root of the directory of the current QRiS project.

...
Anyone can author a protocol by creating or editing a protocol XML file and submitting it for review as a pull request to the Riverscapes XML repository. The Riverscapes Consortium development team will get pull request notifications and action them in a timely manner. See the [contributing protocols](#contributing-protocols) section below for more information on how to contribute a protocol.
...
:::warning
Protocols that have been deprecated will still display previously collected data in their respective DCEs. However, the deprecated protocol will no longer display in the `Properties` window and the user will lose the ability to collect protocol data in DCEs outside of the ones with existing data. 
:::
...
Users can contribute protocols by making a copy of the Riverscapes XML repository and submitting a pull request with the new protocol XML file. The QRiS development team will review the pull request and provide feedback as needed. Once the pull request is approved, the new protocol will be available to all QRiS users after the next synchronization of resources.

1. Protocol author forks the RiverscapesXML repository into their own GitHub account and creates a new branch for the protocol development. See the "Fork" button in the top right corner of the [RiverscapesXML repository](https://github.com/Riverscapes/RiverscapesXML). Here is a [video tutorial](https://www.youtube.com/watch?v=nT8KGYVurIU).
2. Author creates or edits a protocol XML file in the `QRiS/protocols` directory of their forked repository, following the protocol XML schema defined above.
3. Author submits a pull request to the main RiverscapesXML repository with a description of the new protocol and any relevant information for the review process.
4. The QRiS development team reviews the pull request, provides feedback, and requests changes if necessary. The author can make changes to the protocol XML file based on the feedback and update the pull request until it is approved.
5. Once the pull request is approved, the new protocol XML file will be merged into the main RiverscapesXML repository and will be available to all QRiS users after the next synchronization of resources.

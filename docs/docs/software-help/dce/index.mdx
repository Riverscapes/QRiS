...
![dce-references](/images/software-help/dce/DCE-References.png)

DCE references allow the user to associate attachments with their Data Capture Event. This can include (but not limited to) website links, reports, figures, graphs, tables, etc. To add an attachment, go to the bottom right hand corner and select one of the options of `Associate Existing Attachments`, `Add New File`, or `Add New Link`. Associating an existing attachment allows you to select from attachments already added to the QRiS project. Adding a new file or link will allow you to add a new attachment to the project and associate it with your DCE. Once an attachment is linked to the DCE, it will appear in the main window. Under the `Purpose` column, you can assign the attachment as either a *Draft Design, Final Design, Reference, Planning, or Other*. To remove an attachment from the DCE, click on the `Disassociate` button. 

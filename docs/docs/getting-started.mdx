...
QRiS is a QGIS plugin. You can install it from the QGIS plugin manager. To do this, open QGIS and go to Plugins > Manage and Install Plugins. Search for QRiS and click the Install button. NOTE: THE RIVERSCAPES VIEWER PLUGIN IS A DEPENDENCY FOR QRIS AND MUST BE INSTALLED FIRST. To do so, search for 'Riverscapes Viewer' under Manage and Install Plugins and install the latest version.
